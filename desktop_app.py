...
def kamulastirma_hektar_fee(hektar: float, yk: float, include_docs: bool, mera_25: bool) -> tuple[float, list[str]]:
...
def mulkiyet_raporu_fee(report_type: str, hektar: float, km: float, parsel_count: int) -> tuple[float, list[str]]:
    if report_type.startswith("İmar ayırma"):
        adet = max(int(parsel_count or 0), 1)
        total = adet * 235.0
        return total, [f"Parsel başı mülkiyet raporu: {adet} x {tr_money(235)} = {tr_money(total)}"]
    if report_type.startswith("Parselasyon"):
        raw_ha = max(float(hektar or 0), 1.0)
        hektar = billed_hectare(raw_ha, minimum=1)
        total = hektar * 166.0
        details = [f"Hektar başı mülkiyet raporu: {hektar:g} ha x {tr_money(166)} = {tr_money(total)}"]
        if hektar != raw_ha:
            details.append(f"Küsuratlı hektar üst tam sayıya tamamlandı: {raw_ha:g} ha -> {hektar:g} ha")
        return total, details
    eff_km = max(float(km or 0), 1.0)
    total = eff_km * 427.0
    return total, [f"Km başı mülkiyet raporu: {eff_km:g} x {tr_money(427)} = {tr_money(total)}"]


def plan_kroki_fee(area_m2: float, yk: float, parsel_turu: str) -> tuple[float, list[str]]:
    area = normalize_area(area_m2)
    if area <= 0:
    ...
    if area <= 1000:
        ...
        base = 7355.0
        detail = "3.000 < A <= 5.000 m²"
    elif area <= 10000:
        base = 9794.0
        detail = "5.000 < A <= 10.000 m²"
    elif area <= 20000:
        base = 13474.0
        detail = "10.000 < A <= 20.000 m²"
    else:
        extra = ((area - 20000.0) / 10000.0) * 3676.0
        base = 13474.0 + extra
        detail = "20.000 m² üstü orantılı hesap"
...
def cins_degisikligi_fee(area_m2: float, yk: float, parsel_turu: str) -> tuple[float, list[str]]:
    area = normalize_area(area_m2)
    if area <= 0:
        return 0.0, ["Yüzölçümü girilmedi."]
    if area <= 1000:
        ...
        extra = ((area - 3000.0) / 1000.0) * 509.0
        base = 4018.0 + 5550.0 + extra
...
def corner_coordinate_fee(count: int) -> tuple[float, str]:
    ...
    if count <= 500:
        return count * 29.0, "29,00 TL / Nokta (1-500)"
    if count <= 1000:
        return count * 24.0, "24,00 TL / Nokta (501-1000)"
    return count * 11.0, "11,00 TL / Nokta (1001+)"
...
class KadastroDesktopApp(tk.Tk):
    ...
    def build_technical_tab(self) -> None:
        ...
        ttk.Label(
            intro,
            text="Minimal ve hızlı giriş akışı: miktarları doğrudan ok tuşlarıyla girin.",
            style="Muted.TLabel",
        ...
        quick = ttk.LabelFrame(content, text="Teknik Bilgi-Belge Kalemleri", padding=12)
        ...
        self.other_item_var = tk.StringVar(value=OTHER_TECH_ITEMS[0].label)
    ...
    def reset_technical(self) -> None:
        ...
        self.other_item_var.set(OTHER_TECH_ITEMS[0].label)
        self.other_selected_entries.clear()
        self.refresh_other_tree()
    ...
    def on_control_type_change(self, _event: object | None = None) -> None:
        ...
        if self.operation_code == "parselasyon":
            ...
            if report_type.startswith("İmar ayırma"):
                ...
                show.add("km")
        elif self.operation_code in {"irtifak", "birlestirme", "aplikasyon", "parsel_yerinde"}:
            show = {"parsel_count"}
        elif self.operation_code == "cins":
            show = {"area", "cins_parsel_turu"}
    ...
    def calculate_control(self) -> None:
        ...
        uses_yk = True
        if calc == "parselasyon":
            raw, details = parselasyon_fee(area_value, yk, self.geriye_var.get(), self.yeni_var.get())
            ...
            raw, fee_details = imar_degisiklik_fee(area_value, yk, effective_parsel_count, self.mera_var.get())
            ...
            if effective_parsel_count != int(self.parsel_count_var.get() or 0):
                details.append("İfraz/tevhit işlemlerinde minimum parsel sayısı 2 olarak alındı.")
                self.parsel_count_var.set(2)
            raw, fee_details = kadastro_degisiklik_fee(area_value, yk, effective_parsel_count, self.mera_var.get())
            details.extend(fee_details)
        elif calc == "kamulastirma":
            subtype = self.kam_subtype_var.get()
            if subtype == "Şeritvari bazlı":
                eff_km = max(float(km_value or 0), 1.0)
                control = eff_km * 4028.0
                docs = eff_km * 892.0 if self.include_docs_var.get() else 0.0
                raw = control + docs
                details = [f"Şeritvari kontrol: {eff_km:g} km x {tr_money(4028)} = {tr_money(control)}"]
                if self.include_docs_var.get():
                    details.append(f"Harita bilgi ve belge: {eff_km:g} km x {tr_money(892)} = {tr_money(docs)}")
                kam_breakdown = {
                    "control": control,
                    "docs": docs,
                ...
                docs_ha = raw_ha
                control = control_ha * 892.0
                docs = docs_ha * 338.0 if self.include_docs_var.get() else 0.0
                ...
                details = [f"Hektar bazlı kontrol: {control_ha:g} ha x {tr_money(892)} = {tr_money(control)}"]
                if raw_ha > 100.0:
                    details.append("Kontrollük kısmında 100 ha üstü için 100 ha sınırı uygulandı.")
                if self.include_docs_var.get():
                    details.append(f"Harita bilgi ve belge: {docs_ha:g} ha x {tr_money(338)} = {tr_money(docs)}")
                if mera_25:
                    details.append("Mera Kanunu özel durumu: %25 tahsil")
            ...
            raw, details = mulkiyet_raporu_fee(
                self.report_type_var.get(), area_value, km_value, self.report_parsel_var.get()
            )
            uses_yk = False
        elif calc == "irtifak":
            uses_yk = False
            ...
            raw = count * 2008.0
            details = [f"İrtifak tesisi/terkini: {count} x {tr_money(2008)} = {tr_money(raw)}"]
            ...
            raw = max(count - 1, 1) * 2227.0
            details.append(f"((n-1) x 2.227 TL) formülü: ({count}-1) x {tr_money(2227)} = {tr_money(raw)}")
        elif calc == "cins":
            raw, details = cins_degisikligi_fee(area_value, yk, self.cins_parsel_turu_var.get())
        elif calc == "plan_kroki":
            raw, details = plan_kroki_fee(area_value, yk, self.plan_parsel_turu_var.get())
        elif calc == "aplikasyon":
            uses_yk = False
            count = max(int(self.parsel_count_var.get() or 0), 1)
            raw = count * 2215.0
            details = [f"Aplikasyon talebi: {count} x {tr_money(2215)} = {tr_money(raw)}"]
        elif calc == "parsel_yerinde":
            uses_yk = False
            count = max(int(self.parsel_count_var.get() or 0), 1)
            raw = 1003.0 + max(count - 1, 0) * 338.0
            details = [
                f"İlk parsel: {tr_money(1003)}",
                f"İlave {max(count-1,0)} parsel x {tr_money(338)} = {tr_money(max(count-1,0)*338)}",
            ]
            ...
            bagimsiz_total = 0.0
            if bagimsiz > 0:
                bagimsiz_total = 2227.0 if bagimsiz <= 2 else 2227.0 + (bagimsiz - 2) * 338.0
            blok_total = blok * 2227.0
            raw = bagimsiz_total + blok_total
            details = [
                f"Bağımsız bölüm düzeltme bedeli: {tr_money(bagimsiz_total)}",
                f"Blok düzeltme bedeli: {blok} x {tr_money(2227)} = {tr_money(blok_total)}",
            ]
        elif calc == "st_stk":
            uses_yk = False
            prices = {
    ...
    def build_admin_tab(self) -> None:
        ...
        list_frame = ttk.Frame(self.tab_admin)
    ...
    def download_excel_template(self) -> None:
        ...
        ws.append(["DOĞU", "MUŞ", "BULANIK", 1.0, 0.9])
        wb.save(target)
        messagebox.showinfo(
            "Bilgi",
            f"Şablon kaydedildi:\n{target}\n\nManuel düzeltmek için listeden il-ilçe satırına çift tıklayarak güncelleyebilirsiniz.",
        )
    ...
    def import_excel_coefficients(self) -> None:
        ...
        try:
            ...
            if normalized[: len(expected)] != expected:
                raise ValueError("Sütunlar beklenen şablonla uyuşmuyor.")
        ...
        if not imported_rows:
            messagebox.showerror("Hata", "Excel dosyasında geçerli kayıt bulunamadı.")
            return
        ...
        self.refresh_control_iller()
        self.refresh_admin_ui()
        messagebox.showinfo("Bilgi", f"{self.selected_year} yılı için {len(imported_rows)} satır katsayı güncellendi.")
